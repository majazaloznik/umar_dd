---
title: "Delo od doma - Admin poročilo"
...
params:
  start_date: !r Sys.Date() - 25
  end_date: !r Sys.Date() - 18
---
...
data <- raw_data  |> 
        ...
        select(-sort_key) |> 
        mutate(Odmor = ifelse(lunch == FALSE, 0, Odmor))
...
data <- data |> 
        mutate(friday = !is.na(data$Datum) & wday(data$Datum) == 6 ) |> 
        mutate(departureStart = if_else(friday & contractType == 8,
                                        hms::as_hms(as.numeric(departureStart) - 30 * 60),
                                        departureStart),
               departureEnd = if_else(friday & contractType == 8,
                                      hms::as_hms(as.numeric(departureEnd) - 60 * 60),
                                      departureEnd))
...
final <- data |> 
        select(-contractType, -arrivalStart, -arrivalEnd, 
               -departureStart, -departureEnd, -lunch, -odmor_check, -friday) |> 
